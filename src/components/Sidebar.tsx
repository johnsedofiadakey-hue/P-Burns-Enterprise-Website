...
export default function Sidebar({ email }: { email?: string | null }) {
  ...
  const links = [
    ...
    { href: '/admin/products', label: 'Products', icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14v10l-8-4m8 4v10" />
      </svg>
    ), category: 'Operations' },
    ...
    { href: '/admin/pre-orders', label: 'Pre-Orders', icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    { href: '/admin/bookkeeping', label: 'Bookkeeping', icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    { href: '/admin/insights', label: 'Market Insights', icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const filteredLinks = links.filter(link => {
    ...
    if ((label === 'reports' || label === 'market insights') && permissions.includes('reports')) return true;
    if ((label === 'settings' || label === 'users' || label === 'activity log') && permissions.includes('settings')) return true;
    ...
    <>
      ...
      <aside className={`w-64 bg-[#111111] text-white flex flex-col fixed inset-y-0 left-0 z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static h-screen`}>
        <div className="p-6 border-b border-[#1A1A1A]">
          <Image src="/logo-transparent.png" alt="P-Burns Logo" width={150} height={50} className="object-contain invert brightness-0" />
        </div>
